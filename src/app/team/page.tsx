...
export default function TeamPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [agentsRes, gwRes, liveRes] = await Promise.all([
        fetch("/api/agents"),
        fetch("/api/gateway"),
        fetch("/api/live"),
      ]);
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-4 mb-4">
                <div
                  className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl ring-2 ${sc.ring} transition-all duration-300 group-hover:scale-105`}
              ...
              {agent.model && agent.model !== "—" && (
                <div className="mt-3 inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-[#12121a] border border-[#2a2a3e]">
                  ...
                  <span className="text-[10px] text-[#9ca3af] font-mono">{agent.model}</span>
                </div>
              )}
            ...
            <div className="flex gap-12 items-start flex-wrap justify-center">
              {agents.map((agent, i) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`w-16 h-16 rounded-2xl flex items-center justify-center text-2xl ring-2 ${sc.ring} transition-all duration-300 hover:scale-110`}
